...
async function extractYearsAsync(data: RawJsonData): Promise<number[]> {
  ...
  if (yearArray.length === 0) {
    ...
    throw new Error('No valid years found in data. Please ensure your file has year columns (e.g., 2021, 2022, 2023) with data.')
  }
...
export async function processJsonDataAsync(
  ...
  try {
    ...
    const metadata: Metadata = {
      market_name: 'Animal Feed Additives Diagnostics Market',
